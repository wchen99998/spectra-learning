...
@torch.no_grad()
def test_masked_latent_loss_uses_unmasked_teacher_intensity():
    torch.manual_seed(0)
    model = PeakSetSIGReg(
        num_peaks=6,
        model_dim=32,
    ...
    metrics_full = model.forward_augmented(
        {
            "fused_mz": fused_mz,
            "fused_intensity": fused_intensity,
            "fused_valid_mask": fused_valid_mask,
    ...
    fused_intensity_zeroed = fused_intensity.masked_fill(fused_masked_positions, 0.0)
    metrics_zeroed = model.forward_augmented(
        {
            "fused_mz": fused_mz,
            "fused_intensity": fused_intensity_zeroed,
            "fused_valid_mask": fused_valid_mask,
            "fused_masked_positions": fused_masked_positions,
            "fused_precursor_mz": fused_precursor_mz,
        }
    )
    diff = (metrics_full["masked_latent_loss"] - metrics_zeroed["masked_latent_loss"]).abs()
