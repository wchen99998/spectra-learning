...
from spectra_learning.models.peak_features import MzTokenEmbedding
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--root",
        type=Path,
        default=Path("experiments/mz_token_ablation"),
    )
    parser.add_argument("--seeds", type=int, nargs="+", default=[66, 67, 68])
    parser.add_argument("--suffix", default="2p5k")
...
def _embedding_geometry(run_dir: Path, device: torch.device) -> dict[str, float]:
    ...
    bin_size = float(config.encoder_mz_token_bin_size)
    ...
    for chunk in peak_mz.split(4096):
        if isinstance(embedder.mz_features, MzTokenEmbedding):
    ...
    for seed in args.seeds:
        ...
        for metric in PAIRED_VALIDATION_METRICS:
            paired_holdout[metric].append(
                float(holdout["metrics"][metric]["token_minus_fourier"])
            )
        for metric in VALIDATION_METRICS:
            paired[metric].append(
                seed_payload["token"]["final"][metric]
                - seed_payload["fourier"]["final"][metric]
            )
            steps = sorted(
                set(seed_payload["fourier"]["validation_curve"])
                & set(seed_payload["token"]["validation_curve"]),
                key=int,
            )
            paired_curve_means[metric].append(
                float(
                    np.mean(
                        [
                            seed_payload["token"]["validation_curve"][step][metric]
        ...
        if args.geometry:
            for metric in seed_payload["fourier"]["geometry"]:
                paired_geometry.setdefault(metric, []).append(
                    seed_payload["token"]["geometry"][metric]
