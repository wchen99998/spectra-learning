...
def read_project_metadata(
    ...
    with h5py.File(source_path, "r") as file:
        ...
        rt_max, log_intensity_mean, log_intensity_std = (
            _run_normalization_stats(
                full_file_ids,
                full_rt,
                full_intensity,
        ...
        arrays["run_log_intensity_mean"] = log_intensity_mean[
            selected_file_ids
        ]
        arrays["run_log_intensity_std"] = log_intensity_std[
            selected_file_ids
        ]
...
def _ensure_metadata_datasets(file: h5py.File, rows: int) -> None:
    dtypes = {
        ...
        "precursor_intensity_zscore": np.float32,
        "precursor_intensity_present": np.bool_,
    }
...
def metadata_manifest() -> dict[str, Any]:
    return {
        ...
        "normalization": {
            ...
            "precursor_intensity": (
                "clip(zscore(log1p(value)), -5, 5) / 5 within file"
            ),
        },
    }
