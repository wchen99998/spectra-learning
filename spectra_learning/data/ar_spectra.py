...
class SpectraARTokenizer:
    ...
    def _write_mz_prefix(
        ...
        row: int,
        position: int,
        precursor_mz_da: float,
    ) -> int:
        level_values, residual = self._mz_codes(
            precursor_mz_da,
            max_mz=self.config.precursor_mz_max,
            level_bin_counts=self.precursor_mz_level_bin_counts,
        )
    ...
    def _write_fragment_peak(
        ...
        for kind, value in zip(self.fragment_mz_level_kinds, level_values, strict=True):
            position = self._write_token(
                token_ids,
                token_kinds,
                loss_mask,
        ...
        return self._write_token(
            ...
            position,
            SpectraARTokenKind.INTENSITY,
            self._unit_interval_code(intensity, self.config.intensity_bins),
            target=True,
        )
    ...
    def _write_token(
        ...
        row: int,
        position: int,
        kind: SpectraARTokenKind,
        value: int = 0,
        *,
        target: bool,
    ) -> int:
        token_ids[row, position] = self.token_id(kind, value)
        token_kinds[row, position] = int(kind)
        loss_mask[row, position] = target
        return position + 1
    ...
    def _mz_codes(
        ...
        *,
        ...
        mz_da = min(max(mz_da, 0.0), max_mz - 1e-6)
        base_mz = 0.0
        level_values = []
        for width, bins in zip(
            ...
            value = min(int((mz_da - base_mz) // width), bins - 1)
            level_values.append(value)
            base_mz += value * width
        residual_fraction = (mz_da - base_mz) / self.config.mz_bin_widths[-1]
        residual = min(
            int(max(residual_fraction, 0.0) * self.config.residual_bins),
            self.config.residual_bins - 1,
        )
        return tuple(level_values), residual

    def _unit_interval_code(self, value: float, bins: int) -> int:
