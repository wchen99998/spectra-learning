...
def pairmixer_fast_full_visible_tokens(config: Any) -> int:
    return int(_config_get(config, "num_peaks", 64)) + 1
...
def _mae_encoder_predictor_visible_tokens(config: Any) -> tuple[int, int]:
    ...
    block_min_len = int(_config_get(config, "jepa_block_min_len", 1))
    allow_target_overlap = bool(_config_get(config, "jepa_allow_target_overlap", False))
...
def resolve_pairmixer_fast_max_visible_tokens(
    ...
    *,
    ...
    if _config_has(config, "pairmixer_fast_max_visible_tokens"):
        raise ValueError(
