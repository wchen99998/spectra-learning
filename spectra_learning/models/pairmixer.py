...
class TriangleAttention(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.q_norm = _build_norm(self.head_dim, eps=norm_eps, affine=False)
        self.k_norm = _build_norm(self.head_dim, eps=norm_eps, affine=False)
...
class AttentionPairBias(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pair_bias = (
            nn.Linear(pair_dim, num_heads, bias=False) if use_pair_bias else None
        )
...
class GatedSingleToPairUpdate(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.left = nn.Linear(single_dim, pair_dim)
        self.right = nn.Linear(single_dim, pair_dim)
        ...
        _init_linear(self.gate, gate=True)

    def forward(
        self,
        single: Float[Tensor, "batch tokens dim"],
        pair: Float[Tensor, "batch tokens tokens pair"],
        pair_mask: Bool[Tensor, "batch tokens tokens"],
    ) -> Float[Tensor, "batch tokens tokens pair"]:
        single_norm = self.single_norm(single)
        left = self.left(single_norm).unsqueeze(2)
        right = self.right(single_norm).unsqueeze(1)
        update = self.out(left * right)
        gate = torch.sigmoid(self.gate(self.pair_norm(pair)))
        update = gate * update
        return update * pair_mask.unsqueeze(-1).to(dtype=update.dtype)
...
class PairMixerBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.tri_mul_out_post_norm = _build_norm(pair_dim, eps=norm_eps)
        self.tri_mul_in = TriangleMultiplicativeUpdate(
            pair_dim,
            direction="incoming",
            norm_eps=norm_eps,
        )
        self.tri_mul_in_post_norm = _build_norm(pair_dim, eps=norm_eps)
        ...
        self.pair_transition_post_norm = _build_norm(pair_dim, eps=norm_eps)
        if self.use_single_to_pair_update:
            ...
            self.single_to_pair_post_norm = _build_norm(pair_dim, eps=norm_eps)
    ...
    def forward(
        ...
        pair = pair + self.drop(
            self.tri_mul_out_post_norm(self.tri_mul_out(pair, pair_mask))
        )
        pair = pair + self.drop(
            self.tri_mul_in_post_norm(self.tri_mul_in(pair, pair_mask))
        )
        pair = pair + self.drop(
            self.pair_transition_post_norm(
