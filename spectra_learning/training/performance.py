...
def compile_forward(
    ...
    compile_mode = (
        str(config.get("compile_mode", "max-autotune"))
        if compile_mode is None
        else compile_mode
    )
    if compile_mode.lower() == "none":
        return
    compile_scope = str(config.get("compile_scope", "module")).lower()
    if compile_scope == "module":
        model.compile(
            mode=compile_mode,
            fullgraph=False,
        )
        logging.info("Compiled full model with torch.compile mode=%s.", compile_mode)
        return
...
def register_bf16_adam_state_hooks(
    ...
    state_dtype = str(config.get("optimizer_state_dtype", "fp32")).lower()
