...
def resolve_training_route(config: Any) -> tuple[str, str]:
    ...
    if task not in {"pretrain", "contrastive", "ar_spectra", "fake_peak"}:
    ...
    if task == "fake_peak" and backend != "torch":
        raise ValueError("fake_peak training requires device_backend='torch'")
