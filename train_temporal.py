...
def _load_pretrained_checkpoint(
    ...
    bad_missing = [k for k in missing if not any(k.startswith(p) for p in allowed_prefixes)]
    if bad_missing:
        raise RuntimeError(
...
def train_temporal(
    ...
    logging.info("Temporal finetuning for %s epochs (%d steps).", num_epochs, total_steps)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Temporal finetuning for peak-set JEPA.")
